...
export default function ResumePage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      Promise.all([
        fetch('/api/user/profile', { headers: { Authorization: `Bearer ${t}` } }),
        fetch('/api/resume/status', { headers: { Authorization: `Bearer ${t}` } }),
      ]).then(async ([profileRes, resumeRes]) => {
        if (profileRes.ok) {
          const d = await profileRes.json();
          if (d.name) setFullName(d.name);
          if (d.portfolioUrl) setPortfolioUrl(d.portfolioUrl);
        }
    ...
    <div className="max-w-3xl mx-auto space-y-8 pb-20">
      ...
      <Card className="border-none shadow-xl rounded-3xl overflow-hidden bg-white dark:bg-slate-900">
        ...
        <CardContent className="p-8 space-y-6">
          ...
          <Input id="resume-upload" type="file" accept="application/pdf" className="hidden" onChange={(e) => e.target.files?.[0] && setFile(e.target.files[0])} />
        </CardContent>
      </Card>
    </div>
  );
