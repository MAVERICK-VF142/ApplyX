...
export async function POST(req: Request) {
  try {
    ...
    const [resumeFetch, profileFetch] = await Promise.all([
      supabaseAdmin.from('resumes').select('resume_text').eq('user_id', auth.user.id).maybeSingle(),
      supabaseAdmin.from('profiles').select('portfolio_url, name').eq('id', auth.user.id).maybeSingle(),
    ]);
